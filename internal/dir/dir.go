...
import (
	...
	"github.com/ddmin/logo-ls/internal/ctw"
	"github.com/ddmin/logo-ls/internal/sysState"
)
...
func New(d *os.File) (*dir, error) {
	...
	for _, v := range files {
		...
		// symlinks
		fullpath, _ := filepath.Abs(d.Name())
		file_fullpath := fmt.Sprintf("%s/%s", fullpath, f.name+f.ext)
		sym, err := os.Readlink(file_fullpath)

		if err == nil {
			if api.FlagVector&api.Flag_g > 0 {
				f.ext += " -> " + sym
			} else {
				f.ext += "@"
			}
